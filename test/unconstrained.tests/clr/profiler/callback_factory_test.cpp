...
#include "unconstrained\clr\profiler\callback_factory.h"
...
{
    TEST_CLASS(callback_factory_test)
    {
        TEST_METHOD(ClassImplementsIClassFactoryToSupportInstantiationByClr)
        {
            Assert::IsTrue(is_base_of<IClassFactory, callback_factory>::value);
        }

        TEST_METHOD(ClassIsConcreteSoThatItCanBeInstantiatedByModule)
        {
            Assert::IsFalse(is_abstract<callback_factory>::value);
        }
        ...
        TEST_METHOD(ClassHasUniqueIdRepresentingCLSID)
        {
            ...
            Assert::IsTrue(clsid == __uuidof(callback_factory));
        }
        ...
        {
            HRESULT result = callback_factory::get_class_object(__uuidof(callback_factory), __uuidof(IClassFactory), nullptr);
        ...
        TEST_METHOD(get_class_object_returns_CLASS_E_CLASSNOTAVAILABLE_when_given_unknown_class_id)
        {
            ...
            HRESULT result = callback_factory::get_class_object(unknownClassId, __uuidof(IClassFactory), &object);
        ...
        TEST_METHOD(get_class_object_returns_E_NOINTERFACE_and_null_object_when_given_unknown_interface_id)
        {
            ...
            void* object = reinterpret_cast<void*>(-1);
            HRESULT result = callback_factory::get_class_object(__uuidof(callback_factory), unknownInterfaceId, &object);
        ...
        TEST_METHOD(get_class_object_returns_IClassFactory_object_when_given_known_class_and_interface_id)
        {
            ...
            HRESULT result = callback_factory::get_class_object(__uuidof(callback_factory), __uuidof(IClassFactory), reinterpret_cast<void**>(&object));
            Assert::AreEqual(S_OK, result);
            Assert::IsNotNull(dynamic_cast<callback_factory*>(object));
        }
        ...
        {
            unique_ptr<callback_factory> sut = make_unique<callback_factory>();
        ...
        {
            shared_ptr<callback_factory> sut = make_shared<callback_factory>();
        ...
        {
            callback_factory* sut = new callback_factory();
            const unsigned long totalReferenceCount = 100000;
        ...
        {
            unique_ptr<IClassFactory> sut = make_unique<callback_factory>();
        ...
        {
            unique_ptr<IClassFactory> sut { new callback_factory() };
